...
import com.zobgo.common.crawler.enums.BasicErrorCodeEnum;
import com.zobgo.common.crawler.exception.BusinessException;
...
public class FileUtil {
    ...
    public static byte[] getBytesFromInputStream(InputStream inputStream) {
        if (inputStream == null){
            log.info("the inputStream is null");
            return null;
        }
        byte[] data;
        ByteArrayOutputStream byteArrayOutputStream = null;
    ...
    public static void createDirWhenNotExists(String dir){
        ...
        if (!file.isDirectory()) {
            ...
            if (!mkdir){
                throw new BusinessException(BasicErrorCodeEnum.UPLOAD_ERROR_E000021);
            }
        }
    }
    ...
    public static String getFileDir(String rootPath, String subPath, String fileName, String fileSuffix){
        log.info("rootPath", rootPath);
        log.info("subPath", subPath);
        log.info("fileName", fileName);
        if (StringUtils.isEmpty(fileSuffix)){
            return String.format("%s/%s/%s",rootPath,subPath,fileName);
        }
        return String.format("%s/%s/%s/%s/",rootPath,subPath,fileName,fileSuffix);
    }
